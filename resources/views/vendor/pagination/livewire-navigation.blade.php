    ...
    <div class="pagination-container">
        ...
        <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="pagination-nav">
            <ul class="pagination-list">
                ...
                @if ($paginator->onFirstPage())
                    <li class="pagination-item">
                        <span class="pagination-link pagination-disabled" aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <svg class="pagination-icon" style="width: 1rem; height: 1rem;" fill="currentColor" viewBox="0 0 20 20">
                ...
                @else
                    <li class="pagination-item">
                        <button wire:click="previousPage" wire:loading.attr="disabled" rel="prev" class="pagination-link" aria-label="{{ __('pagination.previous') }}">
                            <svg class="pagination-icon" style="width: 1rem; height: 1rem;" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                        </button>
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <li class="pagination-item">
                                    <button wire:click="gotoPage({{ $page }})" wire:loading.attr="disabled" class="pagination-link" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">{{ $page }}</button>
                                </li>
                            @endif
                        @endforeach
                    @endif
                ...
                @if ($paginator->hasMorePages())
                    <li class="pagination-item">
                        <button wire:click="nextPage" wire:loading.attr="disabled" rel="next" class="pagination-link" aria-label="{{ __('pagination.next') }}">
                            <svg class="pagination-icon" style="width: 1rem; height: 1rem;" fill="currentColor" viewBox="0 0 20 20">
